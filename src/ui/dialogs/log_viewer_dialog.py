...
class ProductivityChart(QWidget):
    """
    A custom widget that renders a bar chart of daily page counts.
    It's interactive, showing details on hover.
    """
    def __init__(self, theme, parent=None):
        super().__init__(parent)
        self.theme = theme
        self.setMinimumHeight(200)
        self.daily_data = [] # Expects a list of (QDate, pages) tuples
        self.bar_rects = []
    ...
    def paintEvent(self, event):
        ...
        hover_color = QColor(config.lighten_color(self.theme.get('PRIMARY', '#b0c6ff'), 0.2))
        ...
        for i, (qdate, pages) in enumerate(self.daily_data):
            ...
            # X-Axis labels (date) - show only some to avoid clutter
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.hovered_bar_index != -1:
            ...
            # Use our reliable GREEK_DAYS list for the tooltip
...
class DailyLogWidget(QFrame):
    """
    A redesigned card widget to display log entries for a single day.
    """
    ...
    def _format_date(self, date_str_iso):
        """Formats date string using a reliable internal list for day names."""
        try:
    ...
    def _setup_ui(self):
        ...
        # --- Header Section (Date and Total Pages) ---
        header_frame = QFrame()
        ...
        header_layout.addWidget(self.date_label, 1) # Give it stretch factor
...
class LogViewerDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.theme = config.THEMES.get(config.load_config().get("theme", "Blue"))
        ...
        # --- FIX FOR WEEKEND SELECTION and CALENDAR STYLING ---
        weekend_format = QTextCharFormat()
        weekend_format.setForeground(QColor(self.theme.get('ON_SURFACE', '#e5e1e6'))) # Normal text color
        ...
        for date_edit in [self.start_date_edit, self.end_date_edit]:
            ...
            # Apply theme styles to the calendar popup
        ...
        self.chart = ProductivityChart(self.theme)
    ...
    def setup_controls_and_initial_view(self):
        ...
        # Set initial range to last 30 days or all data if less
    ...
    def update_filtered_view(self):
        ...
        # Populate scroll area with cards
        for date_obj in filtered_dates:
            entries = self.full_log_data[date_obj]
